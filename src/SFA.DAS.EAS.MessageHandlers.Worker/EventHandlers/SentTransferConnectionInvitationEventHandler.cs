...
namespace SFA.DAS.EAS.MessageHandlers.Worker.EventHandlers
{
    ...
    public class SentTransferConnectionInvitationEventHandler : MessageProcessor<SentTransferConnectionInvitationEvent>
    {
        ...
        public SentTransferConnectionInvitationEventHandler(IMessageSubscriberFactory subscriberFactory, ILog log, EmployerAccountDbContext dbContext, INotificationsApi notificationsApi, EmployerApprenticeshipsServiceConfiguration employerApprenticeshipsServiceConfiguration) : base(subscriberFactory, log)
        {
            _db = dbContext;
            ...
            var users = _db.Users
                .Where(user =>
                    user.UserAccountSettings.Any(us =>
                        us.AccountId == messageContent.ReceiverAccountId && us.ReceiveNotifications) &&
                    user.Memberships.Any(ms =>
                        ms.AccountId == messageContent.ReceiverAccountId && ms.Role == Role.Owner));
